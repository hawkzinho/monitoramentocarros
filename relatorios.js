...
// Valores guardados para os switches
let _relVals = {
  faturamento: 0,
  lucroBruto: 0,
  despGeraisTotal: 0,
...
function applyRelSwitches() {
  ...
  const { faturamento, lucroBruto, despGeraisTotal, despCarrosTotal, lucroLiquido } = _relVals;

  // Despesas: OFF = só gerais (padrão original), ON = gerais + veículos
  const despExibida = inclDespCarros
    ? despGeraisTotal + despCarrosTotal
    : despGeraisTotal;
  const labelDesp = document.getElementById('labelDespesas');
  if (labelDesp) labelDesp.textContent = inclDespCarros ? 'Despesas Totais' : 'Desp. Gerais';
  setCard('cardDespesas', despExibida, false);

  // Lucro Líquido: OFF = bruto − gerais (padrão), ON = bruto − gerais − desp. veículos
  const lucroExibido = inclLucroCarros
    ? lucroLiquido - despCarrosTotal
    : lucroLiquido;
  ...
  setCard('cardLucroLiquido', lucroExibido, false, true);
}
...
async function initRelatorios() {
  ...
  document.getElementById('toggleDespCarros')?.addEventListener('change', applyRelSwitches);
  document.getElementById('toggleLucroCarros')?.addEventListener('change', applyRelSwitches);
  await loadRelatorio();
}
...
async function loadRelatorio() {
  ...
  try {
    const [r1, r2, r3, r4] = await Promise.all([
      ...
      db.from('despesas_carros').select('carro_id, valor'),                                         // todas (lucro bruto acumulado + tabela)
      db.from('despesas_carros').select('carro_id, valor').gte('data', start).lte('data', end),     // filtradas por mês (switch)
    ...
    const despCarrosTodos = r2.data || [];
    const despCarrosMes   = r3.data || [];
    const despGerais      = r4.data || [];

    // Mapa acumulado (para lucro bruto e tabela de vendidos)
    const despPorCarro = {};
    despCarrosTodos.forEach(d => {
      despPorCarro[d.carro_id] = (despPorCarro[d.carro_id] || 0) + (parseFloat(d.valor) || 0);
    });

    // Mapa filtrado por mês (para o switch)
    ...
    const despCarrosTotal = despCarrosMes.reduce((s, d) => s + (parseFloat(d.valor)||0), 0); // todos os carros, filtrado por mês
    const despesasTotais  = despCarrosTotal + despGeraisTotal;
    const lucroLiquido    = lucroBruto - despGeraisTotal;

    // Guarda para os switches
    _relVals = { faturamento, lucroBruto, despGeraisTotal, despCarrosTotal, lucroLiquido };

    // Cards estáticos
    setCard('cardFaturamento',  faturamento,     false);
    setCard('cardVendidos',     vendidos.length, true);

    // Cards com switch — aplicados pelo toggle
    applyRelSwitches();
